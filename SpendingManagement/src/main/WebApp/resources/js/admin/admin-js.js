...
function deleteSpending(endpoint, id) {
    if (confirm("Bạn chắc chắn xóa chi tiêu dùng này?") === true) {
        let s = document.getElementById(`spinner${id}`);
        let d = document.getElementById(`delete${id}`);
        s.style.display = "inline-block";
        d.style.display = "none";
        ...
        fetch(endpoint, {
            ...
            console.info(res);
            if (res.status === 204) {
                document.getElementById(`personalTransaction${id}`).style.display = "none";
            } else
                alert("Hệ thống đang có lỗi, vui lòng quay lại sau!");
            d.style.display = "inline-block";
        });
...
const btns = document.querySelectorAll('.js-view-spending');
const views = document.querySelector('.js-modal-view-spending');
function adminSpendingView(endpointview) {
    ...
    fetch(endpointview, {
        ...
        const json = xmlToJson(xml);
        console.log(json);
        let js = document.getElementById("js-modal-view-spending");
        js.innerHTML = `
        <div class="col-lg-8">
            <div class="about-text go-to">
                <h3 class=" mb-4">
        ...
        <div class="col-lg-4">
            <div class="about-avatar d-flex justify-content-center" style="height:200px;">
    ...
    modalClosedd.addEventListener('click', hideAccountView);
    views.addEventListener('click', hideAccountView);
    modalContainerss.addEventListener('click', function (event) {
        event.stopPropagation();
    });
    }
